...
### Status log
- **2026-07-08** — **`cyl_pipeline_runs` reaffirmed OUT for v1 — reconciled a roadmap↔design contradiction.** The 2026-07-01 decision ("No `cyl_pipeline_runs` table for v1; Argo/RunAI own status + queue") was being **re-opened by two stale spots**: the A4 tier-row note "trigger direction under active redesign," and the 2026-07-06 A4 design doc, which is **built around** `cyl_pipeline_runs` + `cyl_pipeline_run_scans` + Realtime (§5/§6/§10) and — being dated after 07-01 — read as superseding it. That's what kept resurfacing (incl. [bloom #404](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/404)'s SKIP-LOCKED / pgmq queue options, **considered and declined** for v1). Reaffirmed: **no runs table, no queue-based trigger, no Realtime-on-a-runs-table for v1** — Bloom's `workflows` service submits Workflow CRDs directly; a Bloom-native runs/status/cancel UX stays EPIC #15. Fixed the tier-row note and put a **SUPERSEDED-for-v1 banner** on the design doc's §5/§6/§10.
