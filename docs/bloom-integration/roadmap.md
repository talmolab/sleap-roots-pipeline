...
| **images-downloader** | per-scan stage-in. `bloomctl download --scan-id` exists (#350/#351) but writes the **old** layout (`images/Wave1/…` + `scans.csv`) — **NOT** the new nested `{scan_key}/<frames>` + `{scan_key}.scan_metadata.json` sidecar the warm-predict container discovers and that the write-back RPC resolves `image_ids` against (corrected during the A4 PoC — the sidecar was hand-authored). | EPIC #9 → #10 | 🔵 Remaining: **author the nested layout + sidecar from live `cyl_images`** ([bloom #411](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/411)); non-interactive auth ([#398](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/398)); Argo-stage wiring. |
