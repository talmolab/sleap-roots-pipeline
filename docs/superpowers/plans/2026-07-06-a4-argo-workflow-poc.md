...
- Pin producer images by `@sha256:` digest (not `latest`, and not a bare `sha-<sha>` tag) — per the A4 design's per-run pin requirement. *(Digest pinning landed later, in #70; at the time of this plan both producers were tag-pinned.)*
...
- ~~**Producer Argo-readiness (both producers, reconcile uniformly):** empty-input → exit 0 (silent-green), exit-non-zero-on-any-scan-fail → whole-batch retry, and SIGTERM/graceful-preempt.~~ **RESOLVED — and both factual claims here are now false.** Driver halves shipped 2026-08-21 ([sleap-roots #266](https://github.com/talmolab/sleap-roots/pull/266), [predict #36](https://github.com/talmolab/sleap-roots-predict/pull/36)); the Argo half is **not yet shipped** — [#56](https://github.com/talmolab/sleap-roots-pipeline/issues/56) is still open and its fix is in review as [PR #60](https://github.com/talmolab/sleap-roots-pipeline/pull/60), whose `exit-gate` template is not yet registered on the cluster. (An earlier version of this line said the Argo half "shipped as #56"; corrected 2026-09-16 after checking live state. The *factual corrections* below stand on the driver halves alone and are unaffected.) Corrections: empty input no longer exits 0 — traits exits `1` with no manifest and `3` with one, predict exits `1`, and bloomctl exits `0` only on *zero requested scans*. The producers therefore **deliberately disagree** on empty input, which is why #56's gate reads each producer's own exit code rather than assuming uniformity. All three now emit `3` for partial success and ship SIGTERM handlers. See `docs/superpowers/specs/2026-09-15-partial-success-exit-code-wiring-design.md`.
