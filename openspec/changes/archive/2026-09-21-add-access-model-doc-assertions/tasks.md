...
- [x] 1.1 Create `scripts/check_docs.py` with the same structure as `check_manifests.py`: a `ROOT`
...
- [x] 1.2 Add `scripts/check_all.sh` running both Python suites. It must NOT be wired into
...
- [x] 2.1 Add assertions that none of `docs/cluster-identities.md`, `README.md`,
...
- [x] 2.2 Correct `.claude/skills/runai/SKILL.md` §1: state that `runai` needs the kubeconfig *and*
...
- [x] 3.1 Add assertions that `README.md`'s `runai` auth cell names **both** `KUBECONFIG` and SSO,
...
- [x] 3.2 Correct `README.md`'s `runai` row and identity sentence.
  - Validate: 3.1 PASSes.
- [x] 3.3 Add an assertion that `docs/cluster-identities.md` contains an auth-plane table whose
...
- [x] 3.4 Write the `Two auth planes` section in `docs/cluster-identities.md`, using `no` (not
...
- [x] 4.1 Add an assertion that the auth-planes section names a confirming party and an ISO date.
  - Validate: run before adding the line — **must FAIL**.
- [x] 4.2 Add `Confirmed with the repo owner, 2026-09-15.` to that section.
  - Validate: 4.1 PASSes.

## 5. Inventory matches the manifests (Requirement 2)

- [x] 5.1 Add an assertion that every stage count stated in `docs/cluster-identities.md` equals the
...
- [x] 5.2 Add an assertion that each `priorityClassName` named in the docs matches the manifest.
  - Validate: run before fixing — record whether it passes; the predictor is `high` and the others
    `interactive-preemptible`.
- [x] 5.3 Correct the stage counts in `docs/cluster-identities.md`.
  - Validate: 5.1 and 5.2 PASS.

## 6. No standing cluster-parity claim (Requirement 2, Scenario 3)

- [x] 6.1 Add an assertion that `docs/cluster-identities.md` contains no present-tense parity claim
...
- [x] 6.2 Replace that paragraph with a dated observation plus a pointer to
  `scripts/check_cluster_drift.sh` and issue #58.
  - Validate: 6.1 PASSes.

## 7. Remaining corrections found by review

- [x] 7.1 Fix the intro's "sitting behind them", which asserts the SSO plane sits behind the three
...
- [x] 7.2 Correct "before they can sign in" → membership gates seeing `busch-lab`, not SSO itself.
- [x] 7.3 State that creating Generic secrets is self-service once you have console access.
- [x] 7.4 Add the quota nuance accurately: `deserved` quota 2, preemptible work may exceed it, but
...
- [x] 7.5 Note that a missing secret produces the same `Pending`/hang failure as a missing
  `hostPath` directory, and that the credential the prod dispatcher mounts is the staging one (#17).
- [x] 7.6 Replace the `Up 6 days` uptime with a dated "verified running" statement.

## 8. Superseded plan bullets

- [x] 8.1 Annotate `docs/superpowers/plans/2026-09-15-cluster-identities-and-namespace-drift.md`
...
- [x] 9.1 Run the full suite: `python scripts/check_manifests.py && python scripts/check_docs.py`.
  - Validate: both exit `0`; the 53 pre-existing manifest assertions still pass.
- [x] 9.2 `openspec validate add-access-model-doc-assertions --strict`.
- [x] 9.3 Rewrite the stale design doc and plan from PR #74, whose premises this change corrects.
