...
- [ ] 1.1 Add a header comment block to `sleap-roots-pipeline.yaml` (above the existing
...
- [ ] 1.2 Validate: `argo lint sleap-roots-pipeline.yaml` still passes (comment-only change, but
  confirm nothing was accidentally broken in the YAML structure).

## 2. Cross-link

- [ ] 2.1 Confirm [bloom #737](https://github.com/Salk-Harnessing-Plants-Initiative/bloom/issues/737)
  references this change/design doc for the companion `salk-bloom`-side implementation.
