...
a fractional GPU via a pod-level `gpu-fraction` annotation (not a whole-GPU
`resources.limits.nvidia.com/gpu`), SHALL NOT set `privileged: true` or `runAsUser: 0` on its
`securityContext`, and SHALL retain a `retryStrategy`.
...
- **WHEN** `sleap-roots-predictor-template.yaml` is inspected
- **THEN** `spec.templates[predictor].metadata.annotations` declares `gpu-fraction` with a numeric
  string value between 0 and 1 (exclusive of 0)
